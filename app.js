...
(function guardIndexRoute(){
  const onIndex = !!document.querySelector('main.view');
  if (!onIndex) return;
  const sess = JSON.parse(localStorage.getItem('mototrap_session') || 'null');
  if (!sess?.username) {
...
onReady(() => {
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', () => {
      localStorage.removeItem('mototrap_session');
      window.location.href = 'login.html';
    });
  }
});
